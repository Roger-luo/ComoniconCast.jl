...
function split_leaf_command(fn::JLFunction)
    ...
    args = map(fn.args) do each
        @smatch each begin
            ::Symbol => xcall(ComoniconCast, :JLArgument; name=QuoteNode(each))
            :($name::$type) => xcall(ComoniconCast, :JLArgument;
                name=QuoteNode(name),
            ...
            Expr(:kw, :($name::$type), value) => xcall(ComoniconCast, :JLArgument;
                name=QuoteNode(name),
                type=wrap_type(fn, type),
                require=false,
                default=hint(value),
            )
            :($name...) => xcall(ComoniconCast, :JLArgument;
                name=QuoteNode(name),
                require=false,
                vararg=true
            )
            Expr(:kw, name::Symbol, value) => xcall(ComoniconCast, :JLArgument;
                name=QuoteNode(name),
                require=false,
                default=hint(value),
            )
        ...
        for each in fn.kwargs
            @sswitch each begin
                @case Expr(:kw, name::Symbol, value)
                    push!(options, xcall(ComoniconCast, :JLOption, QuoteNode(name), Any, hint(value)))
